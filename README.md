...
> *Beyond Per-Token Pricing: A Concurrency-Aware Cost Framework for Self-Hosted LLM Inference*
...
- A neutral **catalog reference** panel when the detected model+quant+GPU matches a paper
  benchmark curve (shows theta_max plus the exact workload protocol under which it was measured).
...
> Preprint: *Beyond Per-Token Pricing: A Concurrency-Aware Cost Framework for Self-Hosted LLM Inference* — arXiv ID pending endorsement. Every numerical claim in §5 of the paper can be re-derived directly from `data/master_results.csv` using the groupby snippet in [`data/README.md`](data/README.md).
