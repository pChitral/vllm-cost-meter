...
| --- | --- | --- | --- |
| `/cost` | GET | JSON | Latest snapshot as a flat dict (26 fields) |
...
Flat 26-key dict — see `CostSnapshot.to_dict()` (`cost.py:106-135`). Every numeric field is rounded for readability and may be `null` if not yet observed (e.g., before two scrapes have populated rolling deques).
