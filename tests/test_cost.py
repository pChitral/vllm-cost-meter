...
def test_compute_c_eff_negative_tps_returns_none():
    # A counter reset (server restart) can momentarily yield a negative rolling
    # rate; effective cost must never be reported as negative.
    assert compute_c_eff(gpu_hourly_total=6.98, tps=-100.0) is None
